...
## Abstract
This paper is about the process of developing a computer game in embedded system. The program employs Adafruit_SSD1306 and Adafruit_GFX Library. This pogram also implement finite system machine (FSM) concept in its code. The hardware connection of the system mainly with Teensy microcontroller and OLED display. Push button and potentiometer as input control of this game.

## 1. Introduction
The project is embedded system of a simple computer game titled "balloon vs birds". The objective of the game is the balloon which control by the player, must avoid the incoming birds. Every bird reach to left screen earn score. The balloon lost life when birds hit it and until three hit will game be over. 
The project show how programming proceed and understand importance charateristic of the circuit. The game implement finite system machine (FSM) display state of start screen, end screen, pause screen and running.
...
Link: [Game Operation](https://github.com/jvnsep/Project3OLEDDisplay/blob/master/result/gameop.mp4)
